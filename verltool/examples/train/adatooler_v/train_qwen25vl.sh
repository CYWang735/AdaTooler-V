...
dataset_name=adatooler-v/rl/Rl_data
train_data=[$(pwd)/data/${dataset_name}/train.parquet]
val_data=[$(pwd)/data/${dataset_name}/train.parquet]
model_name=AdaTooler-V/AdaTooler-V-SFT-model
...
lr=1e-6
reward_manager=adatooler_v # should be okay for simple match
...
run_name_postfix="adatooler_rl"
if [ "$enable_agent" = "True" ]; then
    run_name="eval_${reward_manager}-${strategy}-agent-${model_pretty_name}-${rl_alg}-n${n}-b${batch_size}-t${temperature}"
else
    run_name="eval_${reward_manager}-${strategy}-${model_pretty_name}-${rl_alg}-n${n}-b${batch_size}-t${temperature}"
fi
