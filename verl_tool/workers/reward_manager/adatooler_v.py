...
def math_acc_reward(answer_str: str, ground_truth: str) -> float:
    return 1.0 if grade_answer(answer_str, ground_truth) else 0.0
...
def choices_compute_score(predict: str, ground_truth: list, steps = 100000) -> float:
    ...
    ground_truth = [re.sub(r"</?answer>", "", x) for x in ground_truth]
...
def ocr_compute_score(predict_str: str, ground_truth: list, steps = 100000) -> float:
...
def free_form_compute_score(predict_str: str, ground_truth: list, steps = 100000) -> float:
...
def adatooler_reasoner_score(predict_str: str, ground_truth, problem_type: str) -> float:
...
@register("adatooler_v")
class AdatoolerVRewardManager:
    """
    A reward manager for the Adatooler-V.
    ...
    def __init__(self, tokenizer, num_examine, compute_score=None, reward_fn_key='data_source', **kwargs) -> None:
        ...
        self.compute_score = adatooler_reasoner_score
                ...
                for tool_interact in tool_interact_info_i:
                    if "image" in tool_interact:
                        if isinstance(tool_interact['image'], list):
                            tool_interact['image'] = [x[:5] for x in tool_interact['image']]  # crop the image to first 50 characters
                        elif isinstance(tool_interact['image'], str):
                            tool_interact['image'] = tool_interact['image'][:5] # for debug
